...
export async function studySummary(baseDir, studyUID) {
  ...
  if (existingSeriesUIDs.size === actualSeriesUIDs.size &&
  ...
  // Step 6: Extract study query from first series singleton
  let studyQuery = null;
  
  if (firstSeriesSingleton) {
    // Extract study query from the first series singleton
    studyQuery = TagLists.extract(firstSeriesSingleton, 'study', TagLists.StudyQuery);
    
    // Add NumberOfStudyRelatedSeries and NumberOfStudyRelatedInstances
