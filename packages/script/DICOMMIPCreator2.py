# -*- coding: utf-8 -*-
"""
Created on Wed Jan 11 09:26:26 2023
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog = "DICOM MIP Creator",
        description="Reads in DICOM series and creates thin MIP reformats. (c) 2023 iSchemaView, Inc.")
    ...
    parser.add_argument(
        "-flipZ",
        help="flip the input (transverse slices) data upside-down before generating the MIP images",
        default = False,
        action='store_true'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="series number offset of the output DICOM series with respect to the input series number multiplied by 10 or 100",
        type = int,
        action='store'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-mt",
        "--mip_thickness",
        help="Target MIP thickness (in millimeters)",
        action='store',
        required = True,
        type = float
    )
    parser.add_argument(
        "-ms",
        "--mip_step",
        help="Target MIP step (in millimeters)",
        action='store',
        type = float
    )
    parser.add_argument(
        "-a",
        "--axis",
        help="MIP axis (0=coronal, 1=sagittal, 2=transverse)",
        choices = [0,1,2],
        default=2,
        action='store',
        required = False,
    ...
    #sitk_volume, dcm_ds = load_dicoms(filenames, args.verbose)
    ...
    elif args.series_number_offset:
        sn = sn + int(args.series_number_offset)
    else:
        sn = sn + 1
    ...
    if args.axis == 0:
        sitk_volume = sitk.PermuteAxes(sitk_volume, [0,2,1])
          
    elif args.axis == 1:
        sitk_volume = sitk.PermuteAxes(sitk_volume, [1,2,0])

    size1 = sitk_volume.GetSize()
    spacing1 = sitk_volume.GetSpacing()

    mip_thickness = args.mip_thickness
    mip_thickness_orig = args.mip_thickness
    
    mip_step = 0
    mip_step_orig = 0
    if args.mip_step:
        mip_step = args.mip_step
        mip_step_orig = args.mip_step
    else:
        mip_step = spacing1[2]
        mip_step_orig = mip_step
    
    if(mip_step < spacing1[2]):
        mip_step = spacing1[2]
    elif(mip_step > (size1[2]*spacing1[2])):
         mip_step = size1[2]*spacing1[2]


    mip_step_slices = int(np.around(mip_step / spacing1[2]))
    if mip_step_slices < 1:
        mip_step_slices = 1
    if mip_step_slices > size1[2]:
        mip_step_slices = size1[2]

    mip_step_eff = spacing1[2] * mip_step_slices

   
    newNSlices = 0
    cntr1 = 0
    while(cntr1 < size1[2]):
        cntr1 += mip_step_slices
        newNSlices = newNSlices + 1
    
    mip_thickness_slices = int(np.around(mip_thickness / spacing1[2]))
    if mip_thickness_slices < 1:
        mip_thickness_slices = 1
    elif mip_thickness_slices > size1[2]:
        mip_thickness_slices = size1[2]
            
    mip_thickness_eff = mip_thickness_slices * spacing1[2]

    if(args.verbose >= 1):
        print("MIP thickness as nr. of original volume slices: %d" % (mip_thickness_slices))
        print("MIP thickness effective: %.3f mm (original slice thickness %.3f mm)" % (mip_thickness_eff, spacing1[2]))
        
        print("MIP step as nr. of original volume slices: %d" % (mip_step_slices))
        print("MIP step effective: %.3f mm (original slice thickness %.3f mm" % (mip_step_eff, spacing1[2]))
    
        print("Number of output MIP images: %d" % (newNSlices))

    
    if np.abs(spacing1[1] - spacing1[0]) > 0.05:

        if(args.verbose >= 1):
            print("Resampling thick input slices into thinner slices to get square pixels ...")
    ...
    for output_slice in range(newNSlices):
        ...
        if(args.verbose >= 2):
            print("Generating output MIP image %d of %d" % (output_slice+1, newNSlices))

        index2 = [0,0, cntr1]
        
        slab_slices = mip_thickness_slices
        if(cntr1 + slab_slices) > size1[2]:
            slab_slices = size1[2]-cntr1
        
        size2 = [size1[0], size1[1], slab_slices]
    
        slab_for_mip = sitk.RegionOfInterest(sitk_volume, size=size2, index=index2)
    ...
    if mip_step_orig == 0: mip_step_orig = 1
    sr_num_offset = round(mip_thickness * mip_step_orig)

    dcm_ds.SeriesNumber = str(sn + sr_num_offset)
    ...
    if args.axis == 0:
        orientation_plane = 'CORONAL'
    elif args.axis == 1:
        orientation_plane = 'SAGITTAL'
    else:
        orientation_plane = 'AXIAL'

    dcm_ds.ImageType = "DERIVED\\SECONDARY\\" + orientation_plane + "\\MIP_" + str(sr_num_offset + args.axis)
